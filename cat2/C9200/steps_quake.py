...
def ud_print_uut_descriptor():
    return q.ud.print_uut_descriptor()

def ud_print_product_manifest():
    return q.ud.print_product_manifest()

def ud_print_uut_config():
    return q.ud.print_uut_config()

def ud_retrieve():
    return q.ud.retrieve()

def get_mode():
    """:menu: (enable=True, name=GET MODE,   section=Mode, num=00, args={})"""
    return aplib.PASS if q.mode_mgr.current_mode else (aplib.FAIL, 'Cannot get mode.')
...
def goto_mode(mode, **kwargs):
    ...
    """
    return aplib.PASS if q.mode_mgr.goto_mode(mode, **kwargs) else (aplib.FAIL, 'Cannot go to mode: {0}'.format(mode))
...
def linux_set_network_interface(**kwargs):
    ...
    return q.linux.set_network_interface(**kwargs)
...
def linux_tftp(**kwargs):
    ...
    return q.linux.tftp(**kwargs)

def linux_set_terminal_width(**kwargs):
    return q.linux.set_termial_width(**kwargs)
...
def pcamap_read_uut(**kwargs):
    ...
    return q.pcamap.read_uut(**kwargs)
...
def traffic_fmdiags_traffic_test(**kwargs):
    ...
    return q.traffic.fmdiags.traffic_test(**kwargs)
