...
def nyquist_sequence(seq, pf, udd, container):
    ...
    seq.add_step(steps_nyquist.process_register_genealogy,
    ...
    # ------------------------------------------------------------------------------------------------------------------
    # Power ON & Boot
    seq.add_step(steps_nyquist.power_cycle_on, name='POWER ON BOOT', kwargs={}, group_level=5)
...
def subseq__diags(seq, pf, container, udd):
    ...
    diag_seq.add_step(steps_pf.diags_fan_test, name='FAN TEST', kwargs={}, enabled=True)
