...
def standard_switch_common(profile_type):
    ...
    # Area Check
    seq.add_step(steps_franklin.process_area_check, name='AREACHECK', enabled=True)
    ...
    pcamap_seq.add_step(steps_franklin.process_verify_cmpd, name='VERIFY CMPD SPROM',
                        kwargs={'force': False,
    ...
    if True:  # Logic branch for chamber debug (if needed)
        ...
        for i, corner in enumerate(corners):
            ...
            corner_seq_sublevel3.add_step(steps_franklin.power_cycle_on, name='POWER ON BOOT', group_level=5)
            if True:  # Logic branch for chamber debug (if needed)
                corner_seq_sublevel3.add_step(steps_franklin.goto_mode, name='MODE STARDUST', kwargs={'mode': 'STARDUST'})
                corner_seq_sublevel3.add_step(steps_franklin.diags_sysinit, name='DIAG SYSINIT')
                corner_seq_sublevel3.add_step(steps_franklin.diags_vmargin_test, name='VOLTAGE MARGIN', kwargs={'device_instance': 0, 'margin_level': voltage}, enabled=True)
                corner_seq_sublevel3.add_step(steps_franklin.diags_temperature_test, name='UUT TEMPERATURES', kwargs={'temperature_corner': temperature, 'operational_state': 'idle'}, enabled=True)
                corner_seq_sublevel3.add_step(steps_franklin.diags_psu_check, name='PSU CHECK', kwargs={}, enabled=True)
                corner_seq_sublevel3.add_step(steps_franklin.diags_stackrac_test, name='STACKRAC TEST', kwargs={}, enabled=True)
                corner_seq_sublevel3.add_step(steps_franklin.diags_serdeseye_sif_test, name='SERDES EYE SIF TEST', kwargs={}, enabled=True)
                corner_seq_sublevel3.add_step(steps_franklin.diags_serdeseye_nif_test, name='SERDES EYE NIF TEST', kwargs={}, enabled=True)
                # Diag Testlist build-out
                corner_seq_sublevel3_diag = corner_seq_sublevel3.add_sequence('DIAG TESTS')
                corner_seq_sublevel3_diag.add_step(steps_franklin.goto_mode, name='MODE DIAG', kwargs={'mode': 'DIAG'})
                dynamic_sequence_builder.build_diag_testlist_subseq(diag_seq=corner_seq_sublevel3_diag, container=container, udd=udd, step_module=steps_franklin, category=None, enabled=True)
                # Traffic Testlist build-out
                corner_seq_sublevel3_traf = corner_seq_sublevel3.add_sequence('TRAF TESTS')
                corner_seq_sublevel3_traf.add_step(steps_franklin.goto_mode, name='MODE STARDUST', kwargs={'mode': 'STARDUST'})
                dynamic_sequence_builder.build_traffic_cases_subseq(traffic_seq=corner_seq_sublevel3_traf, container=container, udd=udd, step_module=steps_franklin, category=None, enabled=True)
            # Corner wrap-up
            ...
            corner_seq.add_step(steps_franklin.power_off, name='POWER OFF', group_level=aplib.FINALIZATION - 1)
    ...
    final_seq.add_step(steps_franklin.power_off, name='POWER OFF', group_level=aplib.FINALIZATION - 2)
    final_seq.add_step(steps_franklin.clean_up, name='CLEAN UP', group_level=aplib.FINALIZATION - 1)
