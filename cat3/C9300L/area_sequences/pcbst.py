...
# FRANKLIN
#-----------------------------------------------------------------------------------------------------------------------
def franklin_sequence(seq, pf, udd):
    """ Franklin SEQ
    :param seq:
    :param pf:
    :param udd:
    ...
    # ------------------------------------------------------------------------------------------------------------------
    # Power ON & Boot
    seq.add_step(steps_franklin.power_cycle_on, name='POWER ON BOOT', kwargs={}, group_level=5)

    # SubSeq Blocks
    subseq__preprocess_pcamap(seq, pf)
    subseq__prog_hw(seq, pf, uut_config, 'PREPROCESS')
...
# NEWFAMILY
#-----------------------------------------------------------------------------------------------------------------------
def newfamily_sequence(seq, pf, udd):
    uut_config = udd.get('uut_config', {})
    container = aplib.get_my_container_key()
    subseq__init(seq, pf)
    seq.add_step(steps_newfamily.init, name='INIT', kwargs={})
    return seq
...
def subseq__write_pcamap(seq, pf):
    ...
    pcamap_seq.add_step(steps_pf.pcamap_sync_up, name='SYNC UP PCAMAP')
...
def subseq__prog_hw(seq, pf, uut_config, group='STANDARD'):
    ...
    if group == 'STANDARD':
        ...
        if uut_config.get('nic', None):
            image_nic_seq = prog_hw_seq.add_sequence('DEVICE IMAGE PROGRAMMING NIC', enabled=True)
            image_nic_seq.add_step(steps_pf.goto_mode, name='MODE LINUX', kwargs={'mode': 'LINUX'})
            image_nic_seq.add_step(steps_pf.upgrade_nic, name='PROG NIC')
        prog_hw_seq.add_step(steps_pf.power_cycle_on, name='POWER CYCLE REBOOT', kwargs={}, group_level=5)
    elif group == 'PREPROCESS':
        prog_hw_seq.add_step(steps_pf.upgrade_emmc_fw, name='EMMC FW UPGRADE', kwargs={}, enabled=True)
    else:
...
def subseq__diags(seq, pf, container, udd):
    ...
    diag_seq.add_step(steps_pf.diags_rtc_chkset_test, name='RTC CHK SET TEST', kwargs={'force_set': True}, enabled=True)
    diag_seq.add_step(steps_pf.diags_psu_check, name='PSU CHECK', kwargs={})
    diag_seq.add_step(steps_pf.diags_port_status_test, name='PORT STATUS CHECK', kwargs={}, enabled=True)
    diag_seq.add_step(steps_pf.diags_stackrac_test, name='STACKRAC TEST', kwargs={}, enabled=True)
    diag_seq.add_step(steps_pf.diags_temperature_test, name='DIAG TEMPERATURE TEST', kwargs={'temperature_corner': 'AMBIENT', 'operational_state': 'idle'}, enabled=True)
    diag_seq.add_step(steps_pf.diags_vmargin_test, name='DIAG VMARGIN TEST', kwargs={'device_instance': 0, 'margin_level': 'NOMINAL'}, enabled=True)
    ...
    diag_seq.add_step(steps_pf.diags_serdeseye_sif_test, name='SERDES EYE SIF TEST', kwargs={}, enabled=True)
    diag_seq.add_step(steps_pf.diags_serdeseye_nif_test, name='SERDES EYE NIF TEST', kwargs={}, enabled=True)
