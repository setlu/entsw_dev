...
__title__ = "Franklin Steps Module"
__version__ = '2.0.0'
__author__ = ['bborel']
...
def ud_print_uut_descriptor():
    return f.ud.print_uut_descriptor()

def ud_print_product_manifest():
    return f.ud.print_product_manifest()

def ud_print_uut_config():
    return f.ud.print_uut_config()

def ud_retrieve():
    return f.ud.retrieve()

def get_mode():
    """:menu: (enable=True, name=GET MODE,   section=Mode, num=00, args={})"""
    return aplib.PASS if f.mode_mgr.current_mode else (aplib.FAIL, 'Cannot get mode.')

def goto_mode(mode, **kwargs):
    """
    This step has special meaning.
    :menu: (enable=True, name=BTLDR,         section=Mode, num=01, args={'mode': 'BTLDR'})
    :menu: (enable=True, name=BTLDRG,        section=Mode, num=01.1, args={'mode': 'BTLDRG'})
    ...
    """
    return aplib.PASS if f.mode_mgr.goto_mode(mode, **kwargs) else (aplib.FAIL, 'Cannot go to mode: {0}'.format(mode))
...
def peripheral_manual_select(**kwargs):
    ...
    return f.peripheral.manual_select(**kwargs)
...
def linux_set_network_interface(**kwargs):
    ...
    return f.linux.set_network_interface(**kwargs)
...
def linux_tftp(**kwargs):
    ...
    return f.linux.tftp(**kwargs)

def linux_set_terminal_width(**kwargs):
    return f.linux.set_termial_width(**kwargs)
...
def pcamap_read_uut(**kwargs):
    ...
    return f.pcamap.read_uut(**kwargs)
...
def act2_sign_chip(**kwargs):
    """:menu: (enable=True, name=ACT2 SIGN HW, section=IdPro, num=1, args={'menu': True,})
       :menu: (enable=False, name=ACT2-RSA SIGN HW, section=IdPro, num=1, args={'menu': True, 'keytype': 'ACT2-RSA'})
       :menu: (enable=False, name=ACT2-HARSA SIGN HW, section=IdPro, num=1, args={'menu': True, 'keytype': 'ACT2-HARSA'})"""
    return f.act2.sign_chip(**kwargs)

def act2_authenticate(**kwargs):
    """:menu: (enable=True, name=ACT2 AUTH, section=IdPro, num=2, args={'menu': True})"""
    return f.act2.authenticate(**kwargs)

def x509sudi_sign_certificate(**kwargs):
    """:menu: (enable=True, name=X509 SUDI, section=IdPro, num=3, args={'menu': True})
       :menu: (enable=False, name=X509 SUDI sha1, section=IdPro, num=3, args={'menu': True, 'x509_sudi_hash': 'SHA1'})
       :menu: (enable=False, name=X509 SUDI sha256, section=IdPro, num=3, args={'menu': True, 'x509_sudi_hash': 'SHA256'})
       :menu: (enable=False, name=X509 SUDI sha256 2099, section=IdPro, num=3, args={'menu': True, 'x509_sudi_hash': 'CMCA3'})"""
    return f.x509sudi.sign_certificate(**kwargs)
...
def traffic_fmdiags_traffic_test(**kwargs):
    ...
    return f.traffic.fmdiags.traffic_test(**kwargs)
...
def equip_setup(**kwargs):
    ...
    return f.equip.setup(**kwargs)

# ----------------------------------------------------------------------------------------------------------------------
# Data Utility/Debug
# ----------------------------------------------------------------------------------------------------------------------
def process_read_sn_data(**kwargs):
    """:menu: (enable=False, name=RD TST SN DATA, section=Data, num=01, args={'menu': True})"""
    return f.process.read_sn_data(**kwargs)

def process_lookup_mac(**kwargs):
    """:menu: (enable=False, name=LOOKUP MAC, section=Data, num=02, args={'menu': True})"""
    return f.process.lookup_mac(**kwargs)

def process_get_sw_configs(**kwargs):
    """:menu: (enable=False, name=GET SW CONFIG, section=Data, num=03, args={'menu': True})"""
    return f.process.get_sw_configs(**kwargs)

def process_get_container_details(**kwargs):
    """:menu: (enable=False, name=GET CONTAINER, section=Data, num=04, args={'menu': True})"""
    return f.process.get_container_details(**kwargs)

def process_get_lineid_data(**kwargs):
    """:menu: (enable=False, name=GET LINEID, section=Data, num=05, args={'menu': True})"""
    return f.process.get_lineid_data(**kwargs)