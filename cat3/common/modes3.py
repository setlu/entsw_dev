"""
-------------------------------------------------------------
Transition Functions for the Catalyst Series-3 product family
-------------------------------------------------------------
"""
...
# Product Specific
# ----------------
import _ios_manifest3 as ios_manifest

__title__ = "Catalyst Series-3 Mode Module"
__version__ = '2.0.0'
__author__ = 'bborel'
...
def btldr_to_btldrg(pp, **kwargs):
    ...
    boot_result, _ = pp.mode_mgr.wait_for_boot(boot_mode=['BTLDRG'], boot_msg='(?:Booting)|(?:Initializing)')
    if not boot_result:
...
def linux_to_stardust(pp, **kwargs):
    ...
    log.debug("Looking for {0} ...".format(target_dir))
    ...
    log.debug("Searching for diag image...")
    ffiles = pp.linux.get_device_files(sub_dir='', file_filter='stardust.*?', attrib_flags='-')
    final_diag_image = None
    if ffiles:
        ...
        if not strict_image:
            ...
            log.warning("NOT ALL DIAG FEATURES WILL BE AVAILABLE!")
...
def btldr_to_ios(pp, **kwargs):
    ...
    _, sw_ios = pp.ios._get_image_config(ios_customer_pid=ios_customer_pid, ios_test_pid=ios_test_pid)
        ...
        try:
            ...
            while retry_count <= MAX_IOS_RETRY and not boot_result and not aplib.need_to_abort():
                ...
                try:
                    ...
                    log.warning("TIMEOUT waiting for IOS to load!")
...
def __get_robust_load_details(name, pp, uut_conn, uut_prompt, image, image_dirs, device_name, pattern, guess_image=None):
    ...
    if device_name == 'tftp':
        ...
        if not image:
            ...
            if bp_file:
                log.info("BOOT param is present: '{0}'".format(fparams['BOOT']))
