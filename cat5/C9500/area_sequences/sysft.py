...
from .. import steps_adelphi

__product_line__ = 'C9500'
__desc__ = 'SYSFT'
...
def subseq__init(seq):
    ...
    init_seq.add_step(steps_adelphi.init, name='INIT', kwargs={})
    init_seq.add_step(steps_adelphi.ud_retrieve, name="RETRIEVE UUT DESCRIPTOR")
    init_seq.add_step(steps_adelphi.ud_print_uut_descriptor, name='PRINT DESCRIPTOR')
    init_seq.add_step(steps_adelphi.ud_print_uut_config, name='PRINT UUT CONFIG')
    return seq


def subseq__final(seq):
    final_seq = seq.add_sequence('FINAL', finalization=True)
    seq.add_step(steps_adelphi.power_off, name='POWER OFF', kwargs={}, group_level=1000)
    final_seq.add_step(steps_adelphi.clean_up, name='CLEAN UP')
    final_seq.add_step(steps_adelphi.final, name='END')
