...
--------------------------------
Host: fgnavp31
Primary IP   (eth0): 172.30.121.159
...
def entsw_development():
    ...
    ts.assign_pre_sequence(
        sequence_definition='apollo.scripts.entsw.cat3.area_sequences.c3k_all_pre_sequences.pre_syspb_cable')
    ts.add_pid_map(pid='STACK*', sequence_definition='apollo.scripts.entsw.cat3.area_sequences.c3k_syspb_run.cables')
    # Connections
    server = dict(protocol='ssh', host='10.1.1.1', user='gen-apollo', password='Ad@pCr01!')
    ts.add_connection(name='serverSSH_shared1', **server)
    containers = []
    for i in range(1, 13, 1):
        ...
        # Connections
        # The UUT connections are NOT shared since we must avoid interference from other running containers
        cont.add_connection(name='uutTN', host='10.1.1.4', protocol="telnet", port=2003 + i - 1, timeout=120)
        cont.add_connection(name='serverSSH', shared_conn='serverSSH_shared1', **server)
        containers.append(cont)
    return
